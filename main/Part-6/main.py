...
while run:
    ...
    target_camera_x = player_x - SCREEN_WIDTH / 2 + player_width / 2
    target_camera_y = player_y - SCREEN_HEIGHT / 2 + player_height / 2

    camera_x = camera_x + (target_camera_x - camera_x) * camera_smoothing
    camera_y = camera_y + (target_camera_y - camera_y) * camera_smoothing
    ...
    for platform in platforms:
        ...
        screen.blit(platform_scale_image, (platform.x - camera_x, platform.y - camera_y))

    if not player_flip:
        screen.blit(player_image_right, (player_x - camera_x, player_y - camera_y))
    if player_flip:
        screen.blit(player_image_left, (player_x - camera_x, player_y - camera_y))
